...
@Px
fun dpToPx(@Dimension(unit = Dimension.DP) value: Int): Int = (value * metrics.density).toInt()
@Px
fun dpToPx(@Dimension(unit = Dimension.DP) value: Float): Float = value * metrics.density

@Px
fun spToPx(@Dimension(unit = Dimension.SP) value: Int): Int = (value * metrics.scaledDensity).toInt()
@Px
fun spToPx(@Dimension(unit = Dimension.SP) value: Float): Float = value * metrics.scaledDensity

@Dimension(unit = Dimension.DP)
fun pxToDp(@Px px: Int): Int = px / (metrics.densityDpi / DisplayMetrics.DENSITY_DEFAULT)
@Dimension(unit = Dimension.DP)
fun pxToDp(@Px px: Float): Float = px / (metrics.densityDpi / DisplayMetrics.DENSITY_DEFAULT)
...
@Px
fun dp(@Dimension(unit = Dimension.DP) value: Int): Int = (value * metrics.density).toInt()
@Px
fun dp(@Dimension(unit = Dimension.DP) value: Float): Int = (value * metrics.density).toInt()

@Px
fun sp(@Dimension(unit = Dimension.SP) value: Int): Int = (value * metrics.scaledDensity).toInt()
@Px
fun sp(@Dimension(unit = Dimension.SP) value: Float): Int = (value * metrics.scaledDensity).toInt()
...
/**
 * Inline class storing px dimensions and utility for size convert.
 *
 * Still experimental.
 */
@Suppress("EXPERIMENTAL_FEATURE_WARNING")
inline class Size(@Px val px: Int) {

    @get:Dimension(unit = Dimension.DP)
    val dp: Int get() = px / (metrics.densityDpi / DisplayMetrics.DENSITY_DEFAULT)
    @get:Dimension(unit = Dimension.DP)
    val dpF: Float get() = px.toFloat() / (metrics.densityDpi / DisplayMetrics.DENSITY_DEFAULT)
